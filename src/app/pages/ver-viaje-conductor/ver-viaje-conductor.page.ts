...
export class VerViajeConductorPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      if (this.viajeId) {
        this.cargarDetallesViaje(this.viajeId);
      }
    });
  }
  ...
  async cargarDetallesViaje(id: string) {
    try {
      const viajeData = await this.viajeService.getViajeById(id); // Cambiado a getViajeById
      if (!viajeData) {
