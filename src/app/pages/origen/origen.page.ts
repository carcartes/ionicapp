...
export class OrigenPage implements AfterViewInit {
  ...
  origenDireccion: string = ''; // Dirección del origen
  ...
  onSelectSuggestion(suggestion: any) {
    ...
    this.origenDireccion = suggestion.place_name;
    ...
    if (this.map) {
      ...
      this.suggestions = [];
      this.searchQuery = this.origenDireccion; // Actualiza la barra de búsqueda
    }
  }

  // Navega a la siguiente página (destino) y pasa el origen
  goToNextPage() {
    // Guardar la dirección usando el servicio ViajeService
    this.viajeService.setOrigen(this.origenDireccion); // Guardamos solo la dirección
    this.navCtrl.navigateForward('/destino', {
      queryParams: {
        origen: this.origenDireccion // Pasamos la dirección al destino
      }
    });
  }
